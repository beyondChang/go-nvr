...
MediaMTX (formerly rtsp-simple-server) is a zero-dependency real-time media server that acts as a "media router" between publishers and readers. Go NVR uses it to ingest camera streams — especially for CSI cameras that need a streaming pipeline.
...
## Integrating with Go NVR

Configure Go NVR to pull from MediaMTX:
...
Go NVR will connect to MediaMTX's RTSP port and record the stream.
