...
Go NVR 是一个用 Go 语言编写的轻量级网络视频录像机。它将 IP 摄像头的视频流录制为 MP4 片段并保存到磁盘，提供 Web 界面用于查看录像、管理摄像头和访问录制内容。
...
从 [GitHub Releases](https://github.com/beyondChang/go-nvr/releases) 下载对应架构的最新版本：
...
```bash
./go-nvr-amd64 init --password 你的密码
...
> **注意**：无需准备配置文件！Go NVR 在没有配置文件的情况下启动时会自动初始化。
```
...
此脚本会自动下载二进制文件、创建系统用户（`nvr`）、生成配置、安装 systemd 服务并启动。数据目录：`/var/lib/go-nvr`。
...
| `--data-dir` | `/var/lib/go-nvr` | 录像和数据库的数据目录 |
| `--listen` | `:9090` | HTTP 监听地址 |
| `--config` | `go-nvr.yaml` | 配置文件路径 |
...
1. **`go-nvr init --password <密码>`** — 初始化时直接设置哈希密码（推荐）
2. **配置文件明文** — 在 YAML 中设置 `auth.password`，首次启动时自动转换为 `password_hash`
3. **手动生成哈希** — 使用 `go-nvr hash-password <密码>` 生成，粘贴到 `auth.password_hash`

### 默认路径

| 路径 | 说明 |
|------|------|
| `/var/lib/go-nvr/` | 数据目录（录像、数据库） |
| `/var/lib/go-nvr/go-nvr.db` | SQLite 数据库 |
| `go-nvr.yaml` | 配置文件 |

## 添加第一个摄像头

Go NVR 使用**独立的传输协议 + 编码格式**配置摄像头：
...
编辑配置后重启 Go NVR，或通过 Web UI 在运行时添加摄像头。
...
- 检查配置文件语法：`cat go-nvr.yaml`
- 确认数据目录存在且可写：`ls -la /var/lib/go-nvr/`
- 查看日志：`journalctl -u go-nvr -f`
